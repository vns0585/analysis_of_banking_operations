...
def main() -> None:
    ...
    while True:
        try:
            user_choice = int(input(
                ...
                "1. Получить данные для главной страницы (с начала месяца для выбранной даты)\n"
    ...
    if user_choice == 2:
        ...
        while True:
            user_date_time = input("Введите дату в формате YYYY-MM-DD HH:MM:SS или нажмите Enter для использования"
                                   " текущей: ").encode("utf-8", errors="ignore").decode("utf-8").strip()
            ...
            if re.match(r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$", user_date_time):
                ...
                if user_year < 2\
                        or user_month not in range(1, 13)\
                        or user_day not in range(1, 32)\
                        or user_hour not in range(0, 24)\
                        or user_minute not in range(0, 60)\
                        or user_second not in range(0, 60):
